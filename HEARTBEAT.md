...
找到：`status=pending AND to=greninja` 的 task
...
```bash
YEAR_MONTH=$(date +%Y-%m)
JSON_LINE='{"ts":'$(date +%s)'000,"agent":"greninja","event":"task_completed","task_id":"<task_id>","summary":"<任務摘要>"}'
...
```bash
cat > ~/.openclaw/shared-memory/memory/approved/2026-05-26_greninja_task001_report.md << 'REPORT'
# 任務報告：<task_id>
...
sessions_send sessionKey: "agent:roaringmoon:discord:channel:1482561255955763293" message: "📄 greninja 完成 <task_id>，已寫入 shared-memory/memory/approved/ 待你審核"
```
