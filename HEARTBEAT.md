...
```bash
python3 ~/.openclaw/task-queue/heartbeat_check.py --agent greninja --archive-done --human
```
...
檢查：`demands[] | select(.from=="greninja" and .status=="pending")`
...
# 通知 CEO
sessions_send sessionKey: "agent:roaringmoon:discord:channel:1482561255955763293" message: "📋 greninja 提交了新需求：$NEW_DEMAND_ID
What: <要做什麼>
Why: <為什麼需要>
Priority: medium"
```
